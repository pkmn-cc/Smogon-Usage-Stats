...
export function victoryChance(r1: number, d1: number, r2: number, d2: number) {
  ...
  return 1.0 / (1.0 + Math.pow(10.0, (r2 - r1) / 400.0 / Math.sqrt(1.0 + C * (Math.pow(d1, 2.0) + Math.pow(d2, 2.0)))));
}
...
function erfc3(y: number) {
  ...
  const result = ysq * (xnum + P[2][4]) / (xden + Q[2][4]);
